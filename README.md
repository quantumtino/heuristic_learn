...
#### 阶段三：知识审查（Knowledge Reviewer Agent）⭐
- **模型**: qwen-flash（高效审查）
- **功能**: 对生成内容进行严格的事实性审查
- **创新点**:
...
- **批量测试代码** (`batch_tests/`): 包含批量对比测试脚本和可视化脚本，用于评估系统性能
- **测试结果** (`batch_tests/test_results_YYYYMMDD/`): 每次运行批量测试时创建的独立目录，包含测试数据和可视化图表，这些文件不会被上传到git
- **调试测试代码** (`debug_tests/`): 用于开发过程中的调试测试，同样不会被上传到git

**注意**: 所有测试结果数据（JSON报告和PNG图表）都存储在`batch_tests/test_results_YYYYMMDD/`目录中，每个测试运行都会创建新的时间戳目录，确保数据隔离。
